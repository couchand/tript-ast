...
function typeNot(item, typeDef) {
  ...
  if (type === 'object') {
    return notInstanceOf(item, typeName(typeDef))
  }
...
function typeName(ty) {
  return ty.properties._type.enum[0]
}
...
function assignType(name, excludeNodeType) {
  if (excludeNodeType) {
    return []
  }
  ...
  return new ast.ExpressionStatement({
    expression: new ast.AssignmentExpression({
      ...
      expression: new ast.LiteralStringExpression({
        value: name
      })
    })
  })
...
function generateConstructor(name, ty, excludeNodeType) {
  var checks = []
  var assigns = []
  ...
  return new ast.Method({
    ...
    body: new ast.FunctionBody({
      directives: [],
      statements: [checkNew(name)].concat(checks, assigns, assignType(name, excludeNodeType))
    })
  })
}
